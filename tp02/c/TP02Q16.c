...
int insertionSortParcial(Pokemon array[], int n, int k) {
    int cont = 0;
    for(int i = 1; i < n; i++) {
        Pokemon key = array[i];
        int j = (i < k) ? i - 1 : k - 1;

        while (j >= 0 && difftime(array[j].captureDate, key.captureDate) >= 0) {
            if(difftime(array[j].captureDate, key.captureDate) == 0) {
                if(isBigger(array[j].name, key.name)) {
                    array[j + 1] = array[j];
                    j = j - 1;
                }
            } else {
...
int main() {
    ...
    int numComp = insertionSortParcial(p, cont, 10);
    for(int z = 0; z < 10; z++) {
        printPokemon(&p[z]);
    }

    t = clock() - t;
    FILE *arquivo = fopen("844619_insertionSort.txt", "w");
