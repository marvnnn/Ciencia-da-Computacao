...
int selectionRecursiva(Pokemon array[], int z, int i) {
    int cont = 0;
    if(i < z-1) {
        int minIndex = i;
        for(int j = i + 1; j < z; j++) {
        ...
        cont = cont + selectionRecursiva(array, z, i+1);
    }
    return cont;
}
...
int main() {
  ...
  //    printf("%s\n", line);
    ...
    // int numComp = selectionRecursiva(p, cont, 0);
    // for(int z = 0; z < cont; z++) {
    //     printPokemon(&p[z]);
    // }
    // t = clock() - t;
    // FILE *arquivo = fopen("844619_selecaoRecursiva.txt", "w");
