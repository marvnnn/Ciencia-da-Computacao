...
public class TP02Q11 {
    ...
    public static void countingSort(Pokemons[] array, int n) {

        int maiorCaptureRate = getMaior(array, n);
        int[] count = new int[maiorCaptureRate + 1];
        Pokemons[] ordenado = new Pokemons[n];
        ...
        for (int i = n - 1; i >= 0; i--) {
            if (array[i] != null) { 
                ...
                ordenado[position] = array[i];
                count[captureRate]--;
    
                int j = position;
                while (j < n - 1 && ordenado[j + 1] != null && ordenado[j + 1].getCaptureRate() == captureRate) {
                    if (ordenado[j + 1].getName().compareTo(ordenado[j].getName()) < 0) {
                        Pokemons temp = ordenado[j];
    ...
    public static int getMaior(Pokemons[] array, int n) {
        int maior = -1; 
        for (int i = 0; i < n; i++) {
            if (array[i] != null) { 
                int numAtual = array[i].getCaptureRate();
                if (maior < numAtual) {
                    maior = numAtual;
                }
            }
    ...
    public static void main(String[] args) {
        ...
        String arquivoMat = "844619_countingSort.txt";
        int i = 0;
        TP02Q11 classe = new TP02Q11();
        ...
        countingSort(arrayOrdenado, cont);
