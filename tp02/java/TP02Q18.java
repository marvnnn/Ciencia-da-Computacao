...
public class TP02Q18 {
    ...
    public static void swapPokemons(Pokemons[] array, int i, int j) {
        Pokemons tmp = array[i];
        array[i] = array[j];
        array[j] = tmp;
    }

    public static void quicksortParcial(Pokemons[] array, int esq, int dir, int k) {
        int i = esq, j = dir;
        Pokemons pivo = array[(esq + dir) / 2]; // Usar generation como pivô
        while (i <= j) {
            while (array[i].getGeneration() < pivo.getGeneration() || (array[i].getGeneration() == pivo.getGeneration() && isBigger(pivo.getName(), array[i].getName()))) // Menor que o pivô
                i++;
            while (array[j].getGeneration() > pivo.getGeneration() || (array[j].getGeneration() == pivo.getGeneration() && isBigger(array[j].getName(), pivo.getName()))) // Maior que o pivô
                j--;
            if (i <= j) {
                // Trocar os elementos
                swapPokemons(array, i, j); 
                i++;
                j--;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        String arquivoMat = "844619_quickSortParcial.txt";
        int i = 0;
        TP02Q18 classe = new TP02Q18();
        ...
        quicksortParcial(arrayOrdenado, 0, cont-1, 10);
