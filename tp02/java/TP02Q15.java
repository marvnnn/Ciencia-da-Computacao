...
public class TP02Q15 {
    ...
    public static void swapPokemons(Pokemons[] arr, int i, int j) {
        Pokemons tmp = arr[j];
        arr[j] = arr[i];
        arr[i] = tmp;
    }

    public static int selectionParcial(Pokemons[] array, int cont, int k) {
        int conta = 0;
        for(int i = 0; i < k; i++) {
            int menor = i;
            for(int j = menor+1; j < cont; j++) {
                if(isBigger(array[j].getName(), array[menor].getName()) < 0) {
                    menor = j;
                }
            }
            swapPokemons(array, i, menor); 
        }
        return conta;
    }
    public static void main(String[] args) {
        ...
        selectionParcial(arrayOrdenado, cont, 10);
